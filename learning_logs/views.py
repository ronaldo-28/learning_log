...
def topics(request):
    """Show all of the current user's topics, and public topics."""
    if request.user.is_authenticated:
        user_topics = Topic.objects.filter(owner=request.user).order_by('date_added')
        public_topics_others = Topic.objects.filter(public=True).exclude(owner=request.user).order_by('date_added')
    else:
        user_topics = None
